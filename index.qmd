...
::: {.column-screen style="background-image:url(https://upload.wikimedia.org/wikipedia/commons/c/c0/Pierre_Bourdieu_%281%29.jpg); background-size: cover; height: 40vh; background-position: center -80%;"}
:::
...
Median wealth of individuals whose father has a higher secondary or tertiary degree is €75,400 higher than median wealth of individuals whose father has lower education (see top panel). @cr-desc

The median net wealth of individuals with access to more than 100 books at the age of ten (€97,200) is also significantly higher than in the counterfactual group (€63,300). The difference amounts to €33,900 (see middle panel).  @cr-desc

The gap in median net wealth between individuals with respect to the regular attendence of cultural activities in childhood adds up to €63,000 (see bottom panel). @cr-desc
...
: Direct, indirect and total effects of mediation analysis {tbl-colwidths="[28,18,18,18,18]"}
:::
...
Models (1) to (3) show highly significant total effects, indicating an approximate average net wealth increase between 70% and 100% for individuals who experienced different manifestations of cultural capital at a young age. The latent variable approach - considering all three indicators at once - leads to even
more substantial effects (270%). In contrast, the direct effect is rarely statistically significant, thus
signaling full mediation. This indicates that a high number of books at age ten and a father’s high educational attainment primarily influence net wealth via education and income.
...
densdf <- bind_rows(subdf |> mutate(sample = "Full sample"),
        ...
        d.fambg10_culture = ifelse(d.fambg10_culture == 2, "Yes", "No"),
        d.fambg10_culture = fct_inorder(d.fambg10_culture)) |> 
  select(i.netwealth, d.fambg10_culture, sample, ihs_i.netwealth) 

densdf |> 
  ggplot(aes(x = ihs_i.netwealth, color = d.fambg10_culture, fill = d.fambg10_culture)) +
  geom_density() + 
  facet_grid(sample~d.fambg10_culture, switch = "y") +
  geom_text(data = densdf |> 
            summarise(mm = median(ihs_i.netwealth), medw = median(i.netwealth), .by = c(d.fambg10_culture, sample)),
  ...
  geom_segment(aes(y = 0, yend = 0.015, x = mm, xend = mm), color = "white", linewidth = 1.5,
               data = densdf |> summarise(mm = median(ihs_i.netwealth), .by = c(d.fambg10_culture, sample)))  + 
  scale_fill_manual(values = c("No" = "royalblue4", "Yes" = "coral4"), 
                    aesthetics = c("color","fill")) +
  scale_x_continuous(sec.axis = sec_axis(~., name = "Attended cultural activities"))+
...
We further distinguish between a younger and older sample with a threshold of 50 years of age. This figure shows the descriptive results for the attendence of cultural activities in childhood. @cr-age

We find differences between the age groups also in the multivariate analysis. The results suggest distinct cohort effects with particularly strong effects for the older age group and a less pronounced and only partially significant nexus for younger individuals. @cr-age

The role of cultural capital may thus have altered during the course of social change, for instance, due to educational expansion and technological change that may have facilitated access to cultural capital and reduced intergenerational persistence. @cr-age
...
The webpage is based on an article written by Eva Six and Matthias Schnetzer<br>that has been published in [Review of Economics of the Household](https://doi.org/10.1007/s11150-024-09733-6).
